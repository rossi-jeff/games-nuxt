<template>
	<div class="code-breaker-guess-form">
		<div class="form-color-selects">
			<div v-for="(e, index) in props.columns" :key="e" class="color-select-div">
				<select v-model="state.selected[index]" @change="checkReady" class="color-select" :id="`color-select-${index}`">
					<option
...
div.code-breaker-guess-form {
	@apply flex flex-wrap border justify-between border-black rounded p-2 mx-2 my-1;
}
div.form-color-selects {
	@apply flex flex-wrap justify-between flex-grow p-2;
}
select {
	@apply border border-black rounded px-2 py-1 bg-inherit;
}
</style>
